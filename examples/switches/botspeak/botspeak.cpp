...
BotSpeakBridge::Value::Value(BotSpeakBridge* bridge, const std::string& arg) {
    ...
    if(parts.size() == 2) {
        if(isNumber(parts[1])) {
            ...
            assert(varSize != 0);
            if(index >= varSize) {
                cerr << FormatableString("Attempting to access variable %0 at index %1 past its "
                                         "size %2, accessing last element %3 instead")
                            .arg(parts[0])
                            .arg(index)
                            .arg(varSize)
                            .arg(varSize - 1)
...
#define args1                                                                                          \
    (isSize(args.at(1)) ?                                                                              \
         FormatableString("%0").arg(variablesMap.at(UTF8ToWString(baseNameSize(args.at(1)))).second) : \
         args.at(1))
...
void BotSpeakBridge::startNextOperationIfPossible() {
    if(!currentOperation.get() && !getValue.get() && !nextOperationsOp.empty()) {
        ...
        if(nextOperationsOp.front() == "GET") {
            ...
            currentOperation.reset(
                new Operation(this, nextOperationsOp.front(), nextOperationsArg0.front(), nextOperationsArg1.front()));
            if(verbose)
...
std::wstring BotSpeakBridge::asebaCodeHeader() const {
    ...
    code += L"\tcall leds.circle(DIO[0]*32, DIO[1]*32, DIO[2]*32, DIO[3]*32, DIO[4]*32, DIO[5]*32, "
            L"DIO[6]*32, DIO[7]*32)\n";
...
void BotSpeakBridge::defineVar(const wstring& varName, unsigned varSize) {
    ...
    if(requiredSize > variablesSize)
        throw runtime_error(WStringToUTF8(WFormatableString(L"Error, not enough space in target for variable %0 of "
                                                            L"size %1: requiring %2 words, only %3 available")
                                              .arg(varName)
                                              .arg(varSize)
                                              .arg(requiredSize)
                                              .arg(variablesSize)));
...
//! create an Aseba variable for the corresponding BotSpeak variable, manage _SIZE, optionally give
//! a size
