...
public class DashelSerial {
    ...
    void RequestPermission() throws InterruptedException  {
        PendingIntent mPermissionIntent = PendingIntent.getBroadcast(QtNative.activity(), 0, new Intent(ACTION_USB_PERMISSION), 0);
        IntentFilter filter = new IntentFilter(ACTION_USB_PERMISSION);
        QtNative.activity().registerReceiver(mUsbReceiver, filter);
        manager.requestPermission(device, mPermissionIntent);
        latch.await();
    }

    public DashelSerial() throws InterruptedException {
        manager = (UsbManager) QtNative.activity().getSystemService(Context.USB_SERVICE);
        device = (UsbDevice)  QtNative.activity().getIntent().getParcelableExtra(UsbManager.EXTRA_DEVICE);
        if(device == null) {
