...
static std::string property_as_string(io_registry_entry_t ioRegistryEntry,
                              const char* propertyKey) {
...
void serial_acceptor_service::handle_request_by_active_enumeration() {
    ...
    while ((service = IOIteratorNext(matchingServices)))
    {
        ...
        if(std::find(m_known_devices.begin(), m_known_devices.end(), path) != m_known_devices.end())
            continue;
