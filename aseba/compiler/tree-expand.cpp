 ...
 *   - Nodes inherited from AbstractTreeNode are expanded into "real" nodes (one that can emit
 * bytecode)
 *   - However, data and memory access nodes (TupleVectorNode and MemoryVectorNode) are not
 * expanded, as they will be expanded during the second pass
 *   - Memory management rule: if a node must transform itself into another node, it create the new
 * node, reparent the children to the new node, and return the newly created node. It is the
 * responsability of the parent to remark the new node and delete the orphaned child (generically
 * implemented in Node::expandAbstractNodes())
 *
 * Ex:
 *                  i++                                             i = i + 1
 *
 ...
 * TupleVectorNode (1)
 *
 *     In this case, UnaryArithmeticAssignmentNode will create the new AssignmentNode, reparent its
 * child, create the new subtree, and return the pointer to AssignmentNode. The parent of
 * UnaryArithmeticAssignmentNode must delete its old child, and perform the substitution with the
 * new one. Failing to do so will result in memory leaks.
 */
...
//! Expand "vector (opop)" to "vector (op)= 1", and then call for the expansion of this last
//! expression
Node* UnaryArithmeticAssignmentNode::expandAbstractNodes(std::wostream* dump) {
    ...
    std::unique_ptr<ArithmeticAssignmentNode> assignment(
        new ArithmeticAssignmentNode(sourcePos, arithmeticOp, memoryVector, constant.get()));
    constant.release();
 ...
 *   - Nodes performing operations on vectors are expanded into several equivalent operations on
 * scalars
 *   - Memory management rule: To make it simple, the whole tree is duplicated (each node as to copy
 * itself, or create new nodes to replace it). Then the old tree is deleted as a whole. I agree,
 * this could be more subtle, but it is the easiest solution. A similar mechanism to pass 1 could be
 * considered.
 *
 * Ex:                                                                         buffer[0] = 1
 ...
 * ---------------
 *                         ---------------------                 |               |            | | |
 * |             StoreNode     ImmediateNode  StoreNode   ImmediateNode ImmediateNode (1)
 * ImmediateNode(2)    (buffer[0])         (1)      (buffer[1])        (2)
 *
 */
...
Node* AssignmentNode::expandVectorialNodes(std::wostream* dump, Compiler* compiler, unsigned int) {
    ...
    for(unsigned int i = 0; i < leftVector->getVectorSize(); i++) {
        ...
        block->children.push_back(new AssignmentNode(sourcePos, leftVector->expandVectorialNodes(dump, compiler, i),
