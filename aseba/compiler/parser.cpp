...
//! Check if next token is an unsigned 12 bits integer literal. If so, return it, if not, throw an
//! exception
...
//! Check if next token is an unsigned 16 bits integer literal. If so, return it, if not, throw an
//! exception
...
//! Check if next token is the positive part of a 16 bits signed integer literal. If so, return it,
...
//! Check if next token is the absolute part of a 16 bits signed integer literal. If so, return it,
//! if not, throw an exception
...
unsigned Compiler::expectPositiveConstant() const {
    ...
    if(value < 0 || value > 32767)
        throw TranslatableError(tokens.front().pos, ERROR_PCONSTANT_OUT_OF_RANGE).arg(tokens.front().sValue).arg(value);
    return value;
}
...
int Compiler::expectConstant() const {
    ...
    if(value < -32768 || value > 32767)
        throw TranslatableError(tokens.front().pos, ERROR_CONSTANT_OUT_OF_RANGE).arg(tokens.front().sValue).arg(value);
    return value;
}

//! Check and return either the positive part of a 16 bits signed integer or the value of a valid
//! constant
...
Node* Compiler::parseAssignment() {
    ...
    static const Token::Type compoundBinaryAssignment[] = {
        Token::TOKEN_OP_ADD_EQUAL,        Token::TOKEN_OP_NEG_EQUAL,     Token::TOKEN_OP_MULT_EQUAL,
        Token::TOKEN_OP_DIV_EQUAL,        Token::TOKEN_OP_MOD_EQUAL,     Token::TOKEN_OP_BIT_AND_EQUAL,
        Token::TOKEN_OP_BIT_OR_EQUAL,     Token::TOKEN_OP_BIT_XOR_EQUAL, Token::TOKEN_OP_SHIFT_LEFT_EQUAL,
    ...
    if(tokens.front() == Token::TOKEN_ASSIGN) {
        ...
        std::unique_ptr<ArithmeticAssignmentNode> assignment(ArithmeticAssignmentNode::fromArithmeticAssignmentToken(
...
Node* Compiler::parseFor() {
    ...
    blockNode->children.push_back(new AssignmentNode(rangeStartIndexPos, variable.release(),
...
Node* Compiler::parseCondition() {
    /*const Token::Type conditionTypes[] = { Token::TOKEN_OP_EQUAL, Token::TOKEN_OP_NOT_EQUAL,
    Token::TOKEN_OP_BIGGER, Token::TOKEN_OP_BIGGER_EQUAL, Token::TOKEN_OP_SMALLER,
    ...
    const Token::Type conditionTypes[] = {Token::TOKEN_OP_EQUAL,   Token::TOKEN_OP_NOT_EQUAL,
                                          Token::TOKEN_OP_BIGGER,  Token::TOKEN_OP_BIGGER_EQUAL,
                                          Token::TOKEN_OP_SMALLER, Token::TOKEN_OP_SMALLER_EQUAL};
...
Node* Compiler::parseUnaryExpression() {
    const Token::Type acceptableTypes[] = {
        Token::TOKEN_PAR_OPEN, Token::TOKEN_BRACKET_OPEN,   Token::TOKEN_OP_NEG,     Token::TOKEN_OP_BIT_NOT,
        Token::TOKEN_STR_abs,  Token::TOKEN_STRING_LITERAL, Token::TOKEN_INT_LITERAL};
