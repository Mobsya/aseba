...
Node* BinaryArithmeticNode::optimize(std::wostream* dump) {
    ...
    // multiplications/division by 1, addition/substraction of 0, OR with 0 or false, AND with -1 or
    // true
    {
...
//! Recursively apply de Morgan law as long as nodes are logic operations, and then invert
//! comparisons
...
Node* ArrayReadNode::optimize(std::wostream* dump) {
    ...
    if(immediateChild) {
        ...
        if(index >= arraySize) {
            throw TranslatableError(sourcePos, ERROR_ARRAY_OUT_OF_BOUND_READ).arg(index).arg(arrayName).arg(arraySize);
        }
...
Node* ArrayWriteNode::optimize(std::wostream* dump) {
    ...
    if(immediateChild) {
        unsigned index = immediateChild->value;
        if(index >= arraySize) {
            throw TranslatableError(sourcePos, ERROR_ARRAY_OUT_OF_BOUND_WRITE).arg(index).arg(arrayName).arg(arraySize);
        }
