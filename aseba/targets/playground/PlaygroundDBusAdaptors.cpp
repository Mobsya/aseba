...
PhysicalObjectInterface::PhysicalObjectInterface(EnkiWorldInterface* enkiWorldInterface, PhysicalObject* physicalObject)
    : QObject(enkiWorldInterface), enkiWorldInterface(enkiWorldInterface), physicalObject(physicalObject) {}
...
Thymio2Interface::Thymio2Interface(EnkiWorldInterface* enkiWorldInterface, AsebaThymio2* thymio)
    : QObject(enkiWorldInterface), enkiWorldInterface(enkiWorldInterface), thymio(thymio) {}
    ...
    if(number > 4) {
        QDBusConnection::sessionBus().send(
            message.createErrorReply(QDBusError::InvalidArgs,
                                     QString("Button number %0 does not exist on Thymio II, it "
                                             "only has 5 buttons numbered from 0 to 4.")
                                         .arg(number)));
        return;
    }
...
EnkiWorldInterface::EnkiWorldInterface(PlaygroundViewer* playground)
    : QDBusAbstractAdaptor(playground), playground(playground) {}
...
QDBusObjectPath EnkiWorldInterface::PhysicalObject(QString number, const QDBusMessage& message) {
    ...
    for(auto it(world->objects.begin()); it != world->objects.end(); ++it) {
        if(ptrToString(*it) == number) {
            ...
            QDBusConnection::sessionBus().registerObject(path.path(), new PhysicalObjectInterface(this, *it),
            ...
            if(thymio2)
                QDBusConnection::sessionBus().registerObject(
                    path.path() + "/thymio2", new Thymio2Interface(this, thymio2), QDBusConnection::ExportAllContents);
