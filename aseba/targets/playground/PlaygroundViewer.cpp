...
void PlaygroundViewer::notifyAsebaEnvironment(const EnvironmentNotificationType type, const std::string& description,
                                              const strings& arguments) {
    if(type == EnvironmentNotificationType::DISPLAY_INFO) {
        if(description == "missing Thymio2 feature") {
            addInfoMessage(tr("You are using a feature not available in the simulator, click here "
                              "to buy a real Thymio."),
                           5.0, Qt::blue, QUrl(tr("https://www.thymio.org/en:thymiobuy")));
        } else
...
void PlaygroundViewer::sceneCompletedHook() {
    ...
    if(energyScoringSystemEnabled) {
        ...
        renderText(16, height() * 7 / 8 - 20,
                   QString("E. in pool: %0 - total score: %1").arg(energyPool).arg(totalScore), font);
    }
    ...
    for(unsigned i = 0; i < LOG_HISTORY_COUNT; ++i) {
        ...
        if(!logText[j].isEmpty()) {
            qglColor(logColor[j]);
            renderText(8, (height() * 7) / 8 + 14 + i * 14,
                       QString::fromStdString(logTime[j].toHumanReadableStringFromEpoch()) + " " + logText[j], font);
        }
    }
}
