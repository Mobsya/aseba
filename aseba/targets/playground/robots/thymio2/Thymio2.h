...
public:
    struct Variables {
        ...
        int16_t sdPresent;
        int16_t proxCommRxIntensities[7];

        int16_t freeSpace[505];
    } variables, variablesOld;

public:
    std::fstream sdCardFile;
    int sdCardFileNumber;
    bool proxcommEnableFlag;

    // Logging of Thymio native function calls
    //! A log entry consists of the number of the Thymio native function and the values of the
    //! arguments
...
public:
    ...
    void proxcommEnable(int16_t enable);
    void setIRMessageReceived(int16_t message);
...
protected:
    ...
    int16_t getProxIntensity(unsigned i) const;
};
