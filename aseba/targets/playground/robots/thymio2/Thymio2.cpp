...
void AsebaThymio2::proxcommEnable(int16_t enable){
...
void AsebaThymio2::controlStep(double dt) {
    ...
    variables.proxCommRxIntensities[0] = getProxIntensity(0);
    variables.proxCommRxIntensities[1] = getProxIntensity(1);
    variables.proxCommRxIntensities[2] = getProxIntensity(2);
    variables.proxCommRxIntensities[3] = getProxIntensity(3);
    variables.proxCommRxIntensities[4] = getProxIntensity(4);
    variables.proxCommRxIntensities[5] = getProxIntensity(5);
    ...
    if (proxcommEnableFlag){
        ...
        if ( newIrMessage ){
            newIrMessage = false;
            variables.proxCommRx = irMessage;
            execLocalEvent(EVENT_PROX_COMM);
        } 
        // SEND_NOTIFICATION(LOG_INFO, "Hello  ", robotName);
        // SEND_NOTIFICATION(LOG_INFO, "proxcommintensity", std::to_string(variables.proxCommRxIntensities[3]));
    }
    
   /*  for (int i=0; i++; i<7)
    {
        if (variables.proxCommRxIntensities[i] > 3000){
        int16_t message = rand() % 255;
        time_t msgtime = time(0);
        Point pos1(pos.x, pos.y);
        Enki::simulatorEnvironment->sendIRMessage(robotName, message, msgtime, pos1);
        variables.proxCommTx = message; 
        } */
    
   /*  if (variables.proxCommRxIntensities[i] > 3000 ){
        Point pos2(pos.x, pos.y);
        int16_t message = Enki::simulatorEnvironment->getIRMessage(robotName, pos2);
        
        if ( message > 0 ){
            variables.proxCommRx = message;
             
        } 
    
     }
    }*/
    // SEND_NOTIFICATION(LOG_INFO, "pos value", pos.toStdString());
    // SEND_NOTIFICATION(LOG_INFO, "pos x value", std::to_string(pos.x));
    // SEND_NOTIFICATION(LOG_INFO, "pos x value", std::to_string((int16_t) pos.x));
...
//! Simulate the behaviour of the Thymio firmware, that is, returning 0 when objects are out of
//! range
int16_t AsebaThymio2::getSaturatedProxHorizontal(unsigned i) const {
    ...
    double minimumRange = sensor->getRange() - 1e-4;
    if(dist >= minimumRange)
        return 0;
    return static_cast<int16_t>(sensor->getValue());
}

int16_t AsebaThymio2::getProxIntensity(unsigned i) const {
    const IRSensor* sensor(nullptr);
    switch(i) {
        case 0: sensor = &infraredSensor0; break;
        case 1: sensor = &infraredSensor1; break;
