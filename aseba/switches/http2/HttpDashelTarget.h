...
namespace Http {
    ...
    /**
     * A HTTP Dashel target represents a connection that was established to one of the specified
     * peers on the Aseba network. It is typically identified by its address which also specifies
     * the target for the corresponding Dashel stream.
     *
     * Every target stores a list of nodes as reported by the integrated descriptions manager over
     * the Aseba network. Nodes also store a list of their defined variables for the currently
     * compiled AESL program, as well as a list of pending variable values requested by HTTP
     * requests.
     */
    ...
    public:
         ...
         */
        virtual bool sendGetVariables(unsigned globalNodeId, const std::vector<std::string>& args,
                                      HttpRequest* request);
        ...
        virtual const std::map<unsigned, Node>& getNodes() const {
            return nodes;
        }
