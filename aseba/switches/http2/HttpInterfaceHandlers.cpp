...
void OptionsHandler::handleRequest(HttpRequest* request, const std::vector<std::string>& tokens) {
    // we only use OPTIONS for CORS preflighting, so just reply to the client that everything is
    // okay
...
NodesHandler::NodesHandler(HttpInterface* interface) : InterfaceHttpHandler(interface) {
    addToken("nodes");
...
ResetHandler::ResetHandler(HttpInterface* interface) : InterfaceHttpHandler(interface) {
    addToken("reset");
    addToken("reset_all");
}
...
void ResetHandler::handleRequest(HttpRequest* request, const std::vector<std::string>& tokens) {
    ...
    for(auto iter = targets.begin(); iter != end; ++iter) {
        ...
        for(auto nodesIter = nodes.begin(); nodesIter != nodesEnd;
            ...
            if(node.name.find("thymio-II") == 0) {  // Special case for Thymio-II. Should we instead just check whether
                                                    // motor.*.target exists?
        ...
        for(auto iter = targets.begin(); iter != end; ++iter) {
            ...
            for(auto nodesIter = nodes.begin();
                ...
                if(ok) {
                    ...
                    errorStream << "Target " << target->getAddress() << " failed to get description for node "
                ...
                if(ok) {
                    ...
                    if(!seenNamedVariables) {
                        ...
                        for(int j = 0; j < numVariables; j++) {
                            part << (j == 0 ? "" : ",") << "\"" << WStringToUTF8(description->namedVariables[j].name)
                                 << "\":" << description->namedVariables[j].size;
                        }
                    }
                    part << "}";
                    ...
                    for(int j = 0; j < numCommonEvents; j++) {
                        part << (j == 0 ? "" : ",") << "\"" << WStringToUTF8(commonDefinitions.events[j].name)
                             << "\":" << commonDefinitions.events[j].value;
                    }
                    part << "}";
...
void VariableOrEventHandler::handleRequest(HttpRequest* request, const std::vector<std::string>& tokens) {
    ...
    set<pair<HttpDashelTarget*, const HttpDashelTarget::Node*> > matchingNodes =
        getInterface()->getNodesByNameOrId(tokens[0]);
    ...
    for(auto iter = matchingNodes.begin();
        ...
        if(commonDefinitions.events.contains(UTF8ToWString(tokens[1]), &eventPos)) {
            ...
            if(target->sendEvent(data)) {
                request->respond();  // or perhaps
                                     // {"return_value":null,"cmd":"sendEvent","name":nodeName}?
            } else {
...
bool FileHandler::checkIfResponsible(HttpRequest* request, const std::vector<std::string>& tokens) const {
    return request->getMethod() == "GET" && doesFileExist(request);
}
...
std::map<std::string, std::string> const FileHandler::suffixMapping = {
    {"css", "text/css"},   {"gif", "image/gif"},      {"htm", "text/html"},       {"html", "text/html"},
    {"jpg", "image/jpg"},  {"js", "text/javascript"}, {"pdf", "application/pdf"}, {"png", "image/png"},
