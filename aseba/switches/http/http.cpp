/*
 ...
 :NODENAME is thymio-II. For example.
   ...
   - curl http://localhost:3000/nodes/thymio-II/motor.left.target/100 sets the speed of the left
 motor
   - curl http://nodes/thymio-II/sound_system/4/10 might record sound number 4 for 10 seconds, if
 such an event were defined in AESL.

 Variables and events are learned from the node description and parsed from AESL source when
 provided. Server-side event (SSE) streams are updated as events arrive. If a variable and an event
 have the same name, it is the EVENT that is called. On a local machine the server can handle 600
 requests/sec with 10 concurrent connections, more (up to 2.5 times more) if the requests are
 pipelined as is the HTTP/1.1 default.

 Start an 'asebadummynode 0' and run 'make test' to execute some basic unit tests. Example Node-RED
 flows can be found in ../../examples/http/node-red.
 ...
 DONE:
    ...
    - POST /nodes/:NODENAME/:VARIABLE (sloppily allows GET
 /nodes/:NODENAME/:VARIABLE/:VALUE[/:VALUE]*)
    ...
    - Aesl program bytecode upload (PUT /nodes/:NODENAME)
      use curl -H 'Content-Type: application/octet-stream' --data-ascii "$(cat vmcode.aesl)" -X PUT
 http://127.0.0.1:3000/nodes/thymio-II
    - accept JSON payload rather than HTML form for updates and events (POST /.../:VARIABLE) and
 (POST /.../:EVENT)
 ...
 This code borrows from the rest of Aseba, especially switches/medulla and
...
//            {
//                cerr << "HttpInterface::run "<< streamsToShutdown.size() <<" streams to shut
//                down"; for (StreamSet::iterator si = streamsToShutdown.begin(); si !=
//                streamsToShutdown.end(); si++)
//                    cerr << " " << *si;
//                cerr << endl;
//            }
...
//        for (StreamResponseQueueMap::iterator i = pendingResponses.begin(); i !=
//        pendingResponses.end(); i++)
            ...
            for(auto node2sub = subs.begin(); node2sub != subs.end(); ++node2sub)
                if(node2sub->second == cmdMsg->dest)  // the dest node is remapped to here, so reverse the map
                {
                    if(node2sub->second != node2sub->second) {
                        const uint16_t oldDest(cmdMsg->dest);     // save previous value
...
void HttpInterface::nodeDescriptionReceived(unsigned targetId) {
    ...
    // verbose
    if(verbose)
        wcerr << this << L" Received description for node " << targetId << " " << getNodeName(targetId)
              << " given nodeId " << nodeId << endl;
...
void HttpInterface::incomingDataAseba(Stream* stream) {
    if(verbose)
        cerr << "new incoming for asebaStream " << stream << " con port "
             << stream->getTargetParameter("connectionPort") << endl;
    ...
    if(eventSubscriptions.size() > 0) {
        ...
        for(auto subscriber = eventSubscriptions.begin();
            ...
            if(subscriber->second.count("*") >= 1 || subscriber->second.count(event_name) >= 1) {
                ...
                appendResponse(subscriber->first, 200, (subscriber->first->sse_todo != 0), reply.str().c_str());
            }
        }
    }
}
...
void HttpInterface::routeRequest(HttpRequest* req) {
    ...
    if(req->tokens[0].find("nodes") == 0) {
        ...
        if(req->tokens.size() <= 1) {  // one named node
        ...
        } else if(req->tokens.size() >= 2 &&
                  req->tokens[1].find("events") == 0) {  // subscribe to event stream for this node
            req->tokens.erase(req->tokens.begin(), req->tokens.begin() + 1);
    ...
    if(req->tokens[0].find("events") == 0) {  // subscribe to event stream for all nodes
        return evSubscribe(req, req->tokens);
    }
    if(req->tokens[0].find("reset") == 0 || req->tokens[0].find("reset_all") == 0) {  // reset nodes
...
void HttpInterface::evNodes(HttpRequest* req, strings& args) {
    ...
    for(auto descIt = nodes.begin(); descIt != nodes.end(); ++descIt) {
        ...
        if(!do_one_node) {
            ...
            json << ",\"aeslId\":"
                 << (nodeToAeslIdSubstitutions.find(nodeId) != nodeToAeslIdSubstitutions.end() ?
                         nodeToAeslIdSubstitutions[nodeId] :
                         nodeId);
        ...
        {
            if(!(int(nodeId) == atoi(args[0].c_str()) || nodeName.find(args[0]) == 0))
            ...
            for(size_t i = 0; i < commonDefinitions[nodeId].constants.size(); ++i)
                json << (i == 0 ? "" : ",") << "\"" << WStringToUTF8(commonDefinitions[nodeId].constants[i].name)
            ...
            for(size_t i = 0; i < commonDefinitions[nodeId].events.size(); ++i)
                json << (i == 0 ? "" : ",") << "\"" << WStringToUTF8(commonDefinitions[nodeId].events[i].name)
        ...
        if(!commonDefinitions[nodeId].events.contains(UTF8ToWString(args[1]), &eventPos)) {
            ...
            if(req->method.find("POST") == 0 || args.size() >= 3) {
                ...
                if(values.size() == 0) {
                    finishResponse(req, 404, "");
                    if(verbose)
                        cerr << req << " evVariableOrEevent 404 can't set variable " << args[0] << ", no values"
                             << endl;
                    continue;
                }
                ...
                if(verbose)
                    cerr << req << " evVariableOrEevent schedule var " << values[0] << "(" << nodeId << "," << start
                         << ") add " << req << " to subscribers" << endl;
                continue;
            }
        } else {
            // this is an event
            ...
            finishResponse(req, 204,
                           "");  // or perhaps {"return_value":null,"cmd":"sendEvent","name":nodeName}?
            continue;
        }
    }
}
    ...
    for(auto descIt = nodes.begin(); descIt != nodes.end(); ++descIt) {
        ...
        for(auto it = asebaStreams.begin(); it != asebaStreams.end(); ++it) {
            ...
            for(auto nodeId : it->second) {
                ...
                stream->flush();
                if(descIt->second.name.find(L"thymio-II") == 0) {  // Special case for Thymio-II. Should we instead just
                                                                   // check whether motor.*.target exists?
                    strings args;
...
Dashel::Stream* HttpInterface::getStreamFromNodeId(const unsigned nodeId) {
    ...
    for(std::map<std::string, unsigned>::const_iterator it = targetsToNodeId.begin(); it != targetsToNodeId.end();
        ++it) {
        if(it->second != nodeId)
            continue;
...
void HttpInterface::aeslLoadFile(const unsigned nodeId, const std::string& filename) {
    ...
    //            wcerr << "cannot read aesl script XML from file " << UTF8ToWString(filename) <<
    //            endl;
    else {
        aeslLoad(nodeId, doc);
        if(verbose)
...
void HttpInterface::aeslLoad(const unsigned nodeId, xmlDoc* doc) {
    ...
    if((obj = xmlXPathEvalExpression(BAD_CAST "/network/constant", context))) {
        ...
        for(int i = 0; i < (nodeset ? nodeset->nodeNr : 0); ++i) {
            ...
            if(name && value)
                commonDefinitions[nodeId].constants.push_back(
    ...
    //            commonDefinitions[nodeId].constants.clear();
    //            allVariables.clear();
    //            throw runtime_error(FormatableString("Error compiling aesl script XML for nodeId
    //            %0").arg(nodeId));
    //        }
...
bool HttpInterface::compileAndSendCode(const unsigned nodeId, const wstring& program) {
    ...
    bool is_failsafe = (program.empty() ||
                        program.compare(UTF8ToWString("<!DOCTYPE aesl-source><network><keywords flag=\"true\"/><node "
                                                      "nodeId=\"1\" name=\"thymio-II\"></node></network>\n")) == 0);
...
void HttpInterface::appendResponse(HttpRequest* req, unsigned status, const bool& keep_open, std::string result) {
    ...
    req->more = keep_open;
    if(verbose)
        cerr << req << " appendResponse " << req->status << " <" << req->result.substr(0, 7) << "...>"
             << (keep_open ? ", keep open" : "") << endl;
}
...
unsigned HttpInterface::updateNodeId(Dashel::Stream* stream, unsigned targetId) {
    ...
    if(it != known.end())
        ...
        while(used.count(newId) != 0 && (newId += 20))
            ;           // unsigned newId will wrap around
        if(newId == 0)  //
            throw runtime_error(FormatableString("Can't allocate an unused node id for target id %0 in stream %1")
                                    .arg(targetId)
                                    .arg(stream));
...
bool HttpRequest::initialize(std::string const& _method, std::string const& _uri, std::string const& _protocol_version,
...
void HttpRequest::incomingData() {
    ...
    while(!headers_done) {
        ...
        if(term != 0) {
            ...
            if(header_field.find("Content-Length: ", 0, 16) == 0 || header_field.find("content-length: ", 0, 16) == 0)
                headers["Content-Length"] = header_field.substr(16, term - 16);
            else if(header_field.find("Connection: ", 0, 12) == 0 || header_field.find("connection: ", 0, 12) == 0)
