...
AsebaNetworkInterface::AsebaNetworkInterface(Hub* hub, bool systemBus)
    : QDBusAbstractAdaptor(hub), hub(hub), systemBus(systemBus), eventsFiltersCounter(0) {
...
void AsebaNetworkInterface::LoadScripts(const QString& fileName, const QDBusMessage& message) {
            ...
            if(element.tagName() == "node") {
                ...
                if(ok) {
                    ...
                    if(result) {
                        ...
                        DBusConnectionBus().send(
                            message.createErrorReply(QDBusError::Failed, QString::fromStdWString(error.toWString())));
                        wasError = true;
                        break;
                    }
    ...
    if(noNodeCount) {
        std::wcerr << QString("%0 scripts have no corresponding nodes in the current network and "
...
void AsebaNetworkInterface::SetVariable(const QString& node, const QString& variable, const Values& data,
    ...
    if(pos == unsigned(-1)) {
        ...
        if(!ok) {
            DBusConnectionBus().send(message.createErrorReply(
                QDBusError::InvalidArgs, QString("variable %0 does not exists in node %1").arg(variable).arg(node)));
            return;
        }
    }
...
QDBusObjectPath AsebaNetworkInterface::CreateEventFilter() {
    ...
    DBusConnectionBus().registerObject(path.path(), new EventFilterInterface(this),
...
void Hub::sendMessage(const Message* message, const Stream* sourceStream) {
    ...
    for(auto it = dataStreams.begin(); it != dataStreams.end(); ++it) {
        ...
        try {
            ...
            // if this stream has a problem, ignore it for now, and let Hub call connectionClosed
            // later.
...
void dumpHelp(std::ostream& stream, const char* programName) {
    ...
    stream << "-l, --loop      : makes the switch transmit messages back to the send, not only "
              "forward them.\n";
