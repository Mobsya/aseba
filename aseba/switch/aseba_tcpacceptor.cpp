...
static const std::map<std::string, aseba_endpoint::endpoint_type> endpoint_type_mapping = {
    ...
    {"E-Puck", aseba_endpoint::endpoint_type::simulated_epuck},
    {"Dummy Node", aseba_endpoint::endpoint_type::simulated_dummy_node},
    {"Aseba Switch", aseba_endpoint::endpoint_type::legacy_switch}};
...
void aseba_tcp_acceptor::accept() {
    ...
    m_monitor.async_listen(m_contact, [this, session](const boost::system::error_code& ec) {
        if(ec) {
            // error ?
            return;
        }
        ...
        if(!m_contact.empty() && m_contact.type() == "aseba") {
            ...
            if(!ec) {
                mLogInfo("[tcp] New aseba node connected: {} on {} (protocol version : {}, type : {})",
                         m_contact.name(), m_contact.endpoint(), protocol_version, type_str);
