...
void handleDashelException(Dashel::DashelException e) {
    switch(e.source) {
        ...
        case Dashel::DashelException::ConnectionFailed:
            ...
            break;
        default:
            QMessageBox::critical(nullptr, QObject::tr("Unexpected Dashel Error"),
...
DashelConnectionDialog::DashelConnectionDialog() {
    ...
    connect(&zeroconf, SIGNAL(zeroconfTargetFound(const Aseba::Zeroconf::TargetInformation&)),
            SLOT(zeroconfTargetFound(const Aseba::Zeroconf::TargetInformation&)));
    connect(&zeroconf, SIGNAL(zeroconfTargetRemoved(const Aseba::Zeroconf::TargetInformation&)),
...
//! Update the port list, if toSelect is not empty, select the added item matching this name, return
...
QListWidgetItem* DashelConnectionDialog::addEntry(const QString& title, const QString& connectionType,
                                                  const QString& dashelTarget, const QString& additionalInfo,
...
void DashelInterface::sendMessage(const Message& message) {
    // this is called from the GUI thread through processMessage() or pingNetwork(), so we must lock
    // the Hub before sending
    lock();
    if(stream) {
        try {
...
DashelTarget::DashelTarget(const QString& commandLineTarget)
 : dashelInterface(commandLineTarget), writeBlocked(false) {
    ...
    connect(&dashelInterface, SIGNAL(messageAvailable(Message*)), SLOT(messageFromDashel(Message*)),
            Qt::QueuedConnection);
...
void DashelTarget::messageFromDashel(Message* message) {
    ...
    if(messageHandler == messagesHandlersMap.end()) {
        ...
        /*else
            qDebug() << QString("Unhandeled non user message of type 0x%0 received from
           %1").arg(message->type, 6).arg(message->source);*/
    } else {
...
ReconnectionDialog::ReconnectionDialog(DashelInterface& dashelInterface)
    : dashelInterface(dashelInterface), counter(0) {
