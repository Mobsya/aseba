...
namespace Aseba {
const QString HelpViewer::DEFAULT_LANGUAGE = QStringLiteral("en");

HelpViewer::HelpViewer(QWidget* parent)
    : QWidget(parent)
    , tmpHelpSubDir(QStringLiteral("aseba-studio-%1").arg(QCoreApplication::applicationPid()))
    ...
    QFile(QStringLiteral(":aseba-doc.qhc")).copy(tmpHelpFileNameHC);
    QFile(QStringLiteral(":aseba-doc.qch")).copy(tmpHelpFileNameCH);
    ...
    if(helpEngine->setupData() == false) {
        ...
        QString message = tr("The help file %0 was not loaded successfully. "
                             ...
                             "Please check your installation, or report a bug.")
                              .arg(tmpHelpFileNameHC)
                              .arg(helpEngine->error());
...
void HelpViewer::showHelp(helpType type) {
    ...
    switch(type) {
        ...
        case HelpViewer::USERMANUAL: filename += QLatin1String("asebausermanual.html"); break;
        case HelpViewer::STUDIO: filename += QLatin1String("asebastudio.html"); break;
        case HelpViewer::LANGUAGE: filename += QLatin1String("asebalanguage.html"); break;
    }
...
bool HelpViewer::readSettings() {
    ...
    QSettings settings;
    result = restoreGeometry(settings.value(QStringLiteral("HelpViewer/geometry")).toByteArray());
    move(settings.value(QStringLiteral("HelpViewer/position")).toPoint());
    return result;
}
...
void HelpViewer::writeSettings() {
    ...
    settings.setValue(QStringLiteral("HelpViewer/position"), pos());
}
