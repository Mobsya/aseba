...
NodeTab::NodeTab(MainWindow* mainWindow, Target* target, const CommonDefinitions* commonDefinitions, const unsigned id,
                 QWidget* parent)
    : QSplitter(parent)
    , ScriptTab(id)
...
MainWindow::MainWindow(QVector<QTranslator*> translators, const QString& commandLineTarget, bool autoRefresh,
                       QWidget* parent)
    : QMainWindow(parent), sourceModified(false), autoMemoryRefresh(autoRefresh) {
    // create target
...
void MainWindow::importMemoriesContent() {
    QString importFileName = QFileDialog::getOpenFileName(this, tr("Import memory content"), "",
                                                          "All Files (*);;CSV files (*.csv);;Text files (*.txt)");
    ...
    if(!nodesNotFound.isEmpty() || !variablesNotFound.isEmpty()) {
        ...
        if(!nodesNotFound.isEmpty()) {
            msg += tr("The following nodes are not present in the current network and their "
                      "associated content was not imported:\n");
            foreach(QString value, nodesNotFound)
                msg += "• " + value + "\n";
        }
        if(!variablesNotFound.isEmpty()) {
            msg += tr("The following variables are not present in the current network and their "
                      "associated content was not imported:\n");
...
void MainWindow::setupConnections() {
    ...
    connect(target, SIGNAL(nodeSpecificError(unsigned, unsigned, QString)),
    ...
    connect(target, SIGNAL(executionModeChanged(unsigned, Target::ExecutionMode)),
...
void MainWindow::regenerateToolsMenus() {
    ...
    for(int i = 0; i < nodes->count(); i++) {
        ...
        if(tab) {
            ...
            act = saveBytecodeMenu->addAction(tr("...of %0").arg(target->getName(tab->nodeId())), tab,
                                              SLOT(saveBytecode()));
            connect(tab, SIGNAL(uploadReadynessChanged(bool)), act, SLOT(setEnabled(bool)));
...
void MainWindow::setupMenu() {
    ...
    fileMenu->addAction(QIcon(":/images/fileopen.png"), tr("&Import memories content..."), this,
                        SLOT(importMemoriesContent()));
