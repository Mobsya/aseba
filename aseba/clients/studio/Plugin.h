...
//! To access private members of the development environment (MainWindow and its children), a plugin
...
struct DevelopmentEnvironmentInterface {
    ...
    //! Set the code of the editor from a sequence of elements (each can be multiple sloc) and
    //! highlight one of these elements
...
struct NodeToolInterface {
    ...
    SavedContent getSaved() const {
        return SavedContent(name, saveToDom());
    }
