...
namespace ThymioVPL {
    ...
    bool Scene::isEmpty() const {
        ...
        if(eventActionsSets.size() > 1 || !eventActionsSets[0]->isEmpty())
            return false;
    ...
    //! Remove everything from the scene, leaving it with no object (hence not usable directly), and
    //! set advanced mode or not
    ...
    void Scene::recomputeSceneRect() {
        ...
        setSceneRect(r.adjusted(-(40 + errorGraphicsItem->boundingRect().width() + 40), -40, 40,
                                Style::addRemoveButtonHeight + Style::blockSpacing + 40));
    ...
    void Scene::recompileWithoutSetModified() {
        ...
        const bool isWarning(isSetLast(lastCompilationResult.errorLine) &&
                             ((lastCompilationResult.errorType == Compiler::MISSING_ACTION) ||
                              (lastCompilationResult.errorType == Compiler::MISSING_EVENT)));
