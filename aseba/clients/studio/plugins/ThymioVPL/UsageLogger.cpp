...
namespace ThymioVPL {
    ...
    void UsageLogger::logBlockAction(BlockActionType type, QString blockName, QString blockType, int row, int elementId,
    ...
    void UsageLogger::logSignal(const QObject* sender, const char* signal, unsigned int senderId,
                                QObject* logicalParent) {
    ...
    void UsageLogger::logBlockMouseMove(QString name, QString type, QGraphicsSceneMouseEvent* event) {
        logMouseAction(MOVE_BLOCK, event->scenePos().x(), event->scenePos().y(), mapButtons(event->button()), nullptr,
                       name.toUtf8().constData(), type.toUtf8().constData());
    }

    void UsageLogger::logBlockMouseRelease(QString name, QString type, QGraphicsSceneMouseEvent* event) {
        logMouseAction(RELEASE_BLOCK, event->scenePos().x(), event->scenePos().y(), mapButtons(event->button()),
                       nullptr, name.toUtf8().constData(), type.toUtf8().constData());
    }
    void UsageLogger::logButtonDrag(QString name, QString type, QMouseEvent* event, QDrag* drag) {
        logMouseAction(DRAG_BUTTON, event->pos().x(), event->pos().y(), mapButtons(event->button()), nullptr,
                       name.toUtf8().constData(), type.toUtf8().constData());
    }

    void UsageLogger::logActionSetDrag(int row, QGraphicsSceneMouseEvent* event, QDrag* drag) {
        logMouseAction(DRAG_ACTION_SET, event->scenePos().x(), event->scenePos().y(), mapButtons(event->button()), &row,
                       nullptr, nullptr);
    }

    void UsageLogger::logDropButton(BlockButton* button, QDropEvent* event) {
        logMouseAction(DROP_BUTTON, event->pos().x(), event->pos().y(), mapButtons(event->mouseButtons()), nullptr,
                       button->getName().toUtf8().constData(), nullptr);
    }

    void UsageLogger::logEventActionSetDrop(int row, QGraphicsSceneDragDropEvent* event) {
        logMouseAction(DROP_ACTION_SET, event->pos().x(), event->pos().y(), mapButtons(event->buttons()), &row, nullptr,
                       nullptr);
    }

    void UsageLogger::logMouseAction(MouseActionType type, double xPos, double yPos, MouseButton button, const int* row,
    ...
    void UsageLogger::logTabletData(const VariablesDataVector& data) {
        ...
        qDebug() << "pos data" << cam_x << cam_y << cam_z << cam_ax << cam_ay << cam_az << thymio_x << thymio_z
                 << thymio_ay;
        ...
        qDebug() << "app state" << recording_duration << timeline_left << timeline_right << selected_set_id
                 << selected_set_time << app_recording << board_is_tracked << thymio_is_tracked;
    ...
    MouseButton UsageLogger::mapButtons(Qt::MouseButton b) {
        MouseButton button;
        switch(b) {
            case Qt::NoButton: button = NO; break;
            case Qt::LeftButton: button = LEFT; break;
            case Qt::RightButton: button = RIGHT; break;
            case Qt::MidButton: button = MIDDLE; break;
            default: button = NO; break;
        }
        return button;
    }
