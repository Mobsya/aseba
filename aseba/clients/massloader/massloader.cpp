...
void MassLoader::loadToTarget(const std::string& target) {
    ...
    while(true) {
        ...
        try {
            ...
            // then run
            run();
            // if we were interested to load only once, step for 1 second at most, then run for 1
            // second, and then stop execution
            if(once) {
            ...
            if(element.tagName() == "node") {
                ...
                if(ok) {
                    ...
                    if(result) {
                        ...
                        stream->flush();
                        wcerr << QString("! %1 bytecodes loaded to target %0, you can disconnect "
                                         "target !")
