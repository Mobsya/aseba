...
QtBootloaderInterface::QtBootloaderInterface(Stream* stream, int dest, int bootloaderDest)
    : BootloaderInterface(stream, dest, bootloaderDest), pagesCount(0), pagesDoneCount(0) {}
...
void ThymioUpgraderDialog::readIdVersion() {
    ...
    try {
        ...
        QMessageBox::warning(this, tr("Cannot connect to Thymio II"),
                             tr("Cannot connect to Thymio II: %1.<p>Most probably another program "
                                "is currently connected to the Thymio II. Make sure that there are "
                                "no Studio or other Upgrader running and try again.</p>")
                                 .arg(e.what()));
        return;
    }
        ...
        while(hub.isMessage()) {
            ...
            if(variables && variables->start == firmwareAddress && variables->variables.size() == 2) {
                ...
                firmwareVersion->setText(
                    tr("Current firmware: %1").arg(versionDevStatusToString(currentVersion, currentDevStatus)));
                hub.clearMessage();
                break;
            }
            hub.clearMessage();
...
void ThymioUpgraderDialog::doFlash() {
    ...
    const int warnRet =
        QMessageBox::warning(this, tr("Pre-upgrade warning"),
                             tr("Your are about to write a new firmware to the Thymio II. Make sure that the robot is "
                                "charged and that the USB cable is properly connected.<p><b>Do not unplug the robot "
                                "during the upgrade!</b></p>Are you sure you want to proceed?"),
...
ThymioUpgraderDialog::FlashResult ThymioUpgraderDialog::flashThread(const std::string& _target,
    ...
    try {
        ...
        return FlashResult(FlashResult::WARNING, tr("Cannot connect to Thymio II"),
                           tr("Cannot connect to Thymio II: %1.<p>Most probably another program is "
                              "currently connected to the Thymio II. Make sure that there are no "
                              "Studio or other Upgrader running and try again.</p>")
                               .arg(e.what()));
    }
    ...
    try {
        ...
        return FlashResult(FlashResult::WARNING, tr("Upgrade Error"),
                           tr("Unable to read Hex file: %1").arg(e.toString().c_str()));
    } catch(BootloaderInterface::Error& e) {
        return FlashResult(FlashResult::FAILURE, tr("Upgrade Error"),
                           tr("A bootloader error happened during the upgrade process: %1").arg(e.what()));
    } catch(Dashel::DashelException& e) {
        return FlashResult(FlashResult::FAILURE, tr("Upgrade Error"),
                           tr("A communication error happened during the upgrade process: %1").arg(e.what()));
    }
    ...
    if(reply->error()) {
        ...
        if(officialDevStatus == unsigned(-1)) {
            ...
            if(officialHexFile.open()) {
                ...
                officialFirmwareText->setText(
                    tr("Official firmware: %1").arg(versionDevStatusToString(officialVersion, officialDevStatus)));
