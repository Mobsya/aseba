...
namespace ThymioVPL {
    ...
    public:
        ...
        const Compiler::CompilationResult& compilationResult() {
        ...
        SetItr setsEnd() {
            return eventActionsSets.end();
        }
        SetConstItr setsBegin() const {
            return eventActionsSets.begin();
        }
        SetConstItr setsEnd() const {
            return eventActionsSets.end();
        }
        unsigned setsCount() const {
            return eventActionsSets.size();
        }
        bool isSetLast(unsigned setId) const;

        unsigned getZoomLevel() const {
            return zoomLevel;
        }
    ...
    protected:
        ...
        // TODO: set this always through a function and emit a signal when it is changed, to update
        // windows title (see issue 154)
        bool sceneModified;
