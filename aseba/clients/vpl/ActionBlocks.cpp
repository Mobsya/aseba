...
namespace ThymioVPL {
    // Move Action
    MoveActionBlock::MoveActionBlock(QGraphicsItem* parent) : Block("action", "move", parent) {
    ...
    QVector<uint16_t> MoveActionBlock::getValuesCompressed() const {
        return QVector<uint16_t>(1, ((sliders[0]->value() + 10) << 8) | (sliders[1]->value() + 10));
    }
    ...
    void ColorActionBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
        bodyColor = QColor(sliders[0]->value() * 5.46875 + 80, sliders[1]->value() * 5.46875 + 80,
    ...
    TopColorActionBlock::TopColorActionBlock(QGraphicsItem* parent) : ColorActionBlock(parent, true) {}

    BottomColorActionBlock::BottomColorActionBlock(QGraphicsItem* parent) : ColorActionBlock(parent, false) {}

    // Sound Action
    SoundActionBlock::SoundActionBlock(QGraphicsItem* parent) : Block("action", "sound", parent), dragging(false) {
        notes[0] = 3;
        durations[0] = 1;
        notes[1] = 4;
        durations[1] = 1;
    ...
    void SoundActionBlock::setNote(unsigned noteIdx, unsigned noteVal) {
        ...
        if(note != noteVal) {
            note = noteVal;
            update();
            USAGE_LOG(logBlockAction(SET_NOTE, getName(), getType(), UsageLogger::getRow(this), noteIdx, nullptr,
                                     &noteVal, nullptr, nullptr));
            emit contentChanged();
        }
    }
    ...
    void SoundActionBlock::setDuration(unsigned noteIdx, unsigned durationVal) {
        ...
        if(duration != durationVal) {
            ...
            update();
            USAGE_LOG(logBlockAction(SET_DURATION, getName(), getType(), UsageLogger::getRow(this), noteIdx, nullptr,
                                     &durationVal, nullptr, nullptr));
            emit contentChanged();
        }
    }
    ...
    TimerActionBlock::TimerActionBlock(QGraphicsItem* parent)
        : Block("action", "timer", parent), dragging(false), duration(1.0) {
    ...
    void TimerActionBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
        ...
        // drawing handle
        {
            static const QPointF points[5] = {QPointF(0, 5), QPointF(35, 12), QPointF(60, 0), QPointF(35, -12),
                                              QPointF(0, -5)};
            painter->save();
    ...
    void TimerActionBlock::mouseReleaseEvent(QGraphicsSceneMouseEvent* event) {
        if(dragging) {
            dragging = false;
            emitUndoCheckpointAndClearIfChanged();
            USAGE_LOG(logBlockAction(TIMER, getName(), getType(), UsageLogger::getRow(this), 0, nullptr, nullptr,
