...
namespace ThymioVPL {
    ...
    Block* Block::deserialize(const QDomElement& element, bool advanced) {
        ...
        for(unsigned i = 0; i < block->valuesCount(); ++i)
            block->setValue(
                i, element.attribute(QString("value%0").arg(i), QString("%0").arg(block->getValue(i))).toInt());
...
// Set transparent block FIXME: test ugly, and should test whether QX11Extras module has been found
// rather than the platform
    ...
    BlockWithBody::BlockWithBody(const QString& type, const QString& name, bool up, QGraphicsItem* parent)
        : Block(type, name, parent), up(up), bodyColor(Qt::white) {}
    ...
    void BlockWithButtonsAndRange::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
        ...
        if(showRangeControl) {
            ...
            if(isAnyClose || isAnyRange) {
                QPolygon highCursor;
                highCursor << QPoint(x + highPos + 23, y) << QPoint(x + highPos - 23, y)
                           << QPoint(x + highPos - 23, y + 23) << QPoint(x + highPos, y + 46)
                           << QPoint(x + highPos + 23, y + 23);
                painter->drawConvexPolygon(highCursor);
            }
            if(isAnyFar || isAnyRange) {
                QPolygon lowCursor;
                lowCursor << QPoint(x + lowPos, y + 50) << QPoint(x + lowPos + 23, y + 50 + 23)
                          << QPoint(x + lowPos + 23, y + 96) << QPoint(x + lowPos - 23, y + 96)
                          << QPoint(x + lowPos - 23, y + 50 + 23);
    ...
    StateFilterBlock::StateFilterBlock(const QString& type, const QString& name, QGraphicsItem* parent)
        ...
        for(uint i = 0; i < 4; i++) {
            GeometryShapeButton* button =
                new GeometryShapeButton(QRectF(-30, -30, 60, 60), GeometryShapeButton::QUARTER_CIRCLE_BUTTON, this,
