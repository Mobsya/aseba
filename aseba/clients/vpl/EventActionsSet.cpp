...
namespace ThymioVPL {
    ...
    //! Return a string representing the event and the state filter, suitable for checking code
    //! duplications
    ...
    QVector<uint16_t> EventActionsSet::getContentCompressed() const {
        ...
        // the next words are the types of the blocks, as uint4, compressed 4-by-4 into uint16_ts,
        // big-endian
    ...
    void EventActionsSet::repositionElements() {
        ...
        addButton->setPos(columnPos + (Style::eventActionsSetColumnWidth - 64) / 2,
                          innerBoundingRect().height() + Style::blockSpacing / 2);
        ...
        // totalWidth = (actions.empty() ? basicWidth : basicWidth + Style::blockWidth +
        // Style::blockSpacing);
    ...
    void EventActionsSet::removeBlock(Block* block) {
        // disconnect the selection setting mechanism, as we do not want this removal to select our
        // set
        disconnect(this, SIGNAL(contentChanged()), this, SLOT(setSoleSelection()));
    ...
    //! Add an action block, optionally at a given position (which must be valid, i.e. >= 0), do not
    //! emit changes or undo checkpoint
    ...
    void EventActionsSet::hoverMoveEvent(QGraphicsSceneHoverEvent* hoverEvent) {
        ...
        // delete button
        if(event && (x >= event->pos().x() + dl) && (x < event->pos().x() + dr)) {
            deleteBlockButton->setPos(event->pos().x() + Style::blockWidth - dw, Style::blockSpacing - dh);
            deleteBlockButton->show();
            removeBlockIndex = -2;
        } else if((x >= actionsStartPos) && (x < actionsStopPos) && (fmod(x - dl - actionsXPos, bstride) < bw) &&
                  ((x - dl - actionsXPos) / bstride < actions.size())) {
            removeBlockIndex = (x - dl - actionsXPos) / bstride;
            deleteBlockButton->setPos(actionsXPos + removeBlockIndex * bstride + Style::blockWidth - dw,
                                      Style::blockSpacing - dh);
            deleteBlockButton->show();
        } else {
            deleteBlockButton->hide();
            removeBlockIndex = -1;
...
// drag and drop start
    ...
    void EventActionsSet::setVisualFromEvent(QGraphicsSceneDragDropEvent* event) {
        if(event->mimeData()->hasFormat("EventActionsSet")) {
            ...
            if(isDnDNewAction(event)) {
                ...
                deleteButton->setPos(currentWidth - deleteButton->boundingRect().width() - Style::blockSpacing,
                                     Style::blockSpacing);
    ...
    void EventActionsSet::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
        ...
        if(vplScene) {
            ...
            /*if (false)
            {
                ...
                painter->setPen(QPen(Style::eventActionsSetBackgroundColors[0], borderWidth,
            Qt::DotLine, Qt::RoundCap, Qt::RoundJoin));
                //painter->drawRoundedRect(innerBoundingRect().adjusted(hb,hb,-hb,-hb), borderWidth,
            borderWidth); painter->drawRoundedRect(innerBoundingRect().adjusted(hb,hb,-hb,-hb),
            Style::eventActionsSetCornerSize, Style::eventActionsSetCornerSize); return;
            }*/
        }
        ...
        if(!actions.empty()) {
            ...
            painter->setPen(
                QPen(Style::eventActionsSetBackgroundColors[0], borderWidth, Qt::DotLine, Qt::RoundCap, Qt::RoundJoin));
            const QRectF rect(QRectF(totalWidth - Style::blockWidth - Style::blockSpacing, 0,
                                     Style::blockWidth + Style::blockSpacing, innerBoundingRect().height())
                                  .adjusted(-hb, hb, -hb, -hb));
        ...
        if(highlightMode == HIGHLIGHT_SET)
            painter->drawRoundedRect(-Style::blockSpacing / 2, 0, currentWidth + Style::blockSpacing,
                                     Style::blockHeight + 2 * Style::blockSpacing, borderWidth, borderWidth);
        else
            painter->drawRoundedRect(innerBoundingRect(), Style::eventActionsSetCornerSize,
                                     Style::eventActionsSetCornerSize);

        // event drop area
        if(!event) {
            drawBlockArea(painter, "event", QPointF(Style::blockSpacing, Style::blockSpacing),
                          highlightMode == HIGHLIGHT_EVENT);
        } else {
            // if event drag&drop
            if(highlightMode == HIGHLIGHT_EVENT) {
                ...
                painter->setPen(Qt::NoPen);
                painter->drawRoundedRect(Style::blockSpacing, Style::blockSpacing / 2, Style::blockWidth,
                                         Style::blockSpacing + Style::blockHeight, borderWidth, borderWidth);
            }
        }
        ...
        painter->drawEllipse(columnPos, ymiddle - 2 * Style::eventActionsSetColumnWidth,
                             Style::eventActionsSetColumnWidth, Style::eventActionsSetColumnWidth);
        painter->drawEllipse(columnPos, ymiddle + Style::eventActionsSetColumnWidth, Style::eventActionsSetColumnWidth,
                             Style::eventActionsSetColumnWidth);
        ...
        */
        ...
        if((actions.empty() || currentWidth == totalWidth) && (actions.size() != 6))
            drawBlockArea(painter, "action", QPointF(dropAreaXPos, Style::blockSpacing),
                          highlightMode == HIGHLIGHT_NEW_ACTION);
        ...
        if(highlightMode == HIGHLIGHT_EXISTING_ACTION) {
            ...
            painter->setPen(Qt::NoPen);
            painter->drawRoundedRect(xpos, Style::blockSpacing / 2, Style::blockWidth,
                                     Style::blockSpacing + Style::blockHeight, borderWidth, borderWidth);
        }
        ...
        painter->drawText(
            QRect(currentWidth - Style::blockSpacing - 128, Style::blockHeight + Style::blockSpacing - 128, 128, 128),
            Qt::AlignRight | Qt::AlignBottom, QString("%0").arg(getRow() + 1));
    ...
    void EventActionsSet::drawBlockArea(QPainter* painter, const QString& type, const QPointF& pos,
        ...
        painter->drawRoundedRect(pos.x() + halfBorderWidth, pos.y() + halfBorderWidth, Style::blockWidth - borderWidth,
                                 Style::blockHeight - borderWidth, borderWidth, borderWidth);
    }
}  // namespace ThymioVPL
}  // namespace Aseba
