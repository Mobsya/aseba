...
namespace ThymioVPL {
    ...
    Compiler::CompilationResult::CompilationResult(ErrorType errorType, int errorLine, int referredLine)
        : errorType(errorType), errorLine(errorLine), referredLine(referredLine) {}
    ...
    QString Compiler::CompilationResult::getMessage(bool advanced) const {
        switch(errorType) {
            ...
            case DUPLICATED_EVENT: {
                if(!advanced)
                    return QObject::tr("The event in line %0 is the same as in line %1")
                        .arg(errorLine + 1)
                        .arg(referredLine + 1);
                else
                    return QObject::tr("The event and the state condition in line %0 are the same "
                                       "as in line %1")
                        .arg(errorLine + 1)
                        .arg(referredLine + 1);
            }
            default: return QObject::tr("Unknown VPL error");
        }
    }
