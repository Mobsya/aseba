...
namespace ThymioVPL {
    ...
    void ThymioVisualProgramming::openHelp() {
        ...
        const AboutBox::Parameters aboutParameters = {
            ...
            tr("The Visual Programming Language (VPL) for the Thymio robot allows to program by "
               "associating event and actions blocks."),
            tr("https://www.thymio.org/en:thymiovpl"),
            "",
            {"core", "vpl", "packaging", "translation"}};
        // we use nullptr here as parent might disappear under our eyes if the Thymio node gets
        // disconnected
    ...
    void ThymioVisualProgramming::saveSnapshot() const {
        ...
        QString initialFileName(settings
    ...
    void ThymioVisualProgramming::setColors(QComboBox* comboBox) {
        ...
        for(unsigned i = 0; i < Style::blockColorsCount(); ++i)
            comboBox->addItem(drawColorScheme(Style::blockColor("event", i), Style::blockColor("state", i),
                                              Style::blockColor("action", i)),
                              "");
    }

    QPixmap ThymioVisualProgramming::drawColorScheme(const QColor& eventColor, const QColor& stateColor,
    ...
    void ThymioVisualProgramming::updateBlockButtonImages() {
        for(QList<BlockButton*>::iterator itr(eventButtons.begin()); itr != eventButtons.end(); ++itr)
            (*itr)->updateBlockImage(scene->getAdvanced());

        for(QList<BlockButton*>::iterator itr(actionButtons.begin()); itr != actionButtons.end(); ++itr)
            (*itr)->updateBlockImage(scene->getAdvanced());
    }
    ...
    QDomDocument ThymioVisualProgramming::transformDomToVersion1(const QDomDocument& document0) {
        ...
        for(QDomElement buttonset0(vplroot0.firstChildElement("buttonset")); !buttonset0.isNull();
            buttonset0 = buttonset0.nextSiblingElement("buttonset")) {
    ...
    static QImage blend(const QImage& firstImage, const QImage& secondImage, qreal ratio) {
        ...
        if(firstImage.format() != secondImage.format())
            return QImage();
        if((firstImage.format() != QImage::Format_ARGB32) &&
           (firstImage.format() != QImage::Format_ARGB32_Premultiplied))
            return QImage();
        ...
        for(int y = 0; y < firstImage.height(); ++y) {
            ...
            for(int x = 0; x < firstImage.width(); ++x) {
                ...
                *pD++ = (((((vF >> 24) & 0xff) * a + ((vS >> 24) & 0xff) * oma) & 0xff00) << 16) |
    ...
    float ThymioVisualProgramming::computeScale(QResizeEvent* event, int desiredToolbarIconSize) {
        ...
        const int uncompressibleHeight(max(actionsLabel->height(), eventsLabel->height()) + desiredToolbarIconSize * 2 +
                                       4 * style()->pixelMetric(QStyle::PM_ButtonMargin) +
                                       4 * style()->pixelMetric(QStyle::PM_DefaultFrameWidth) +
                                       style()->pixelMetric(QStyle::PM_LayoutVerticalSpacing) + 6 * 10 +
        ...
        const int uncompressibleWidth(2 * style()->pixelMetric(QStyle::PM_LayoutHorizontalSpacing) +
    ...
    void ThymioVisualProgramming::resizeEvent(QResizeEvent* event) {
        ...
        int desiredIconSize((event->size().width() -
                             (
                                 ...
                                 style()->pixelMetric(QStyle::PM_LayoutLeftMargin) +
                                 style()->pixelMetric(QStyle::PM_LayoutRightMargin) + 6  // vertical lines
                                                                                         /*#ifdef Q_WS_MAC
