...
namespace ThymioVPL {
    ...
    void Compiler::CodeGenerator::EventHandler::generateAdditionalCode() {
        if(isStateSet)
            code.append(EventHandler::PairCodeAndIndex(L"\tcall math.copy(state, new_state)"
    ...
    Compiler::CodeGenerator::CodeGenerator()
        : advancedMode(false), useSound(false), inWhenBlock(false), inIfBlock(false) {
    ...
    void Compiler::CodeGenerator::link() {
        ...
        for(EventHandlers::iterator it(eventHandlers.begin()); it != eventHandlers.end(); ++it) {
            EventHandler* eventHandler(*it);
            for(QVector<EventHandler::PairCodeAndIndex>::const_iterator jt(eventHandler->code.begin());
                jt != eventHandler->code.end(); ++jt) {
    ...
    void Compiler::CodeGenerator::visit(const EventActionsSet& eventActionsSet, bool debugLog) {
        ...
        if((eventName == "button") &&
           (eventActionsSet.getEventBlock()->getValue(5) != ArrowButtonsEventBlock::MODE_ARROW))
            lookupEventName = "rc5";
    ...
    void Compiler::CodeGenerator::visitEventAndStateFilter(const Block* block, const Block* stateFilterBlock,
                                                           wstring& currentCode) {
        // generate event code
        wstring text;
        ...
        if(block->isAnyValueSet()) {
            ...
            const bool isIf((block->getName() == "button") &&
                            (block->getValue(5) != ArrowButtonsEventBlock::MODE_ARROW));
            if(isIf)
                text += L"\tif ";
            else
                text += L"\twhen ";
        ...
        if(mode == ArrowButtonsEventBlock::MODE_ARROW) {
            ...
            for(unsigned i = 0; i < 5; ++i) {
                if(block->getValue(i) > 0) {
                    static const wchar_t* directions[] = {L"forward", L"left", L"backward", L"right", L"center"};
                    text += (first ? L"" : L" and ");
                    text += L"button.";
                    text += directions[i];
                    text += L" == 1";
            ...
            const unsigned mapping[5][3] = {
                {80, 32, unsigned(-1)}, {85, 17, 77}, {81, 33, unsigned(-1)}, {86, 16, 78}, {87, 13, unsigned(-1)}};
    ...
    wstring Compiler::CodeGenerator::visitActionSetState(const Block* block, bool& isStateSet) {
        ...
        for(unsigned i = 0; i < block->valuesCount(); ++i) {
            switch(block->getValue(i)) {
                case 1:
                    text += indString + L"new_state[" + toWstring(i) + L"] = 1\n";
                    isStateSet = true;
                    break;
                case 2:
                    text += indString + L"new_state[" + toWstring(i) + L"] = 0\n";
                    isStateSet = true;
                    break;
