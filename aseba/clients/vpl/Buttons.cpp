...
namespace ThymioVPL {
    // Clickable button
    GeometryShapeButton::GeometryShapeButton(const QRectF rect, const ButtonType type, QGraphicsItem* parent,
                                             const QColor& initBrushColor, const QColor& initPenColor)
    ...
    void GeometryShapeButton::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*) {
        ...
        switch(buttonType) {
            case CIRCULAR_BUTTON: painter->drawEllipse(boundingRectangle); break;
            ...
            default: painter->drawRect(boundingRectangle); break;
        }
    }
    ...
    void GeometryShapeButton::mousePressEvent(QGraphicsSceneMouseEvent* event) {
        if(event->button() == Qt::LeftButton) {
            if(toggleState) {
                ...
                curState = 1;
                for(QList<GeometryShapeButton*>::iterator itr = siblings.begin(); itr != siblings.end(); ++itr)
                    (*itr)->setValue(0);
            }
            update();
            emit stateChanged();
    ...
    AddRemoveButton::AddRemoveButton(bool add, QGraphicsItem* parent) : QGraphicsObject(parent), add(add) {
    ...
    RemoveBlockButton::RemoveBlockButton(QGraphicsItem* parent) : QGraphicsObject(parent) {
        setAcceptedMouseButtons(Qt::LeftButton);
    }
    ...
    BlockButton::BlockButton(const QString& name, ThymioVisualProgramming* vpl, QWidget* parent)
        : QPushButton(parent), block(Block::createBlock(name)), vpl(vpl) {
