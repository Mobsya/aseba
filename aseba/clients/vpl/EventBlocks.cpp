...
namespace ThymioVPL {
    // Buttons Event
    const QRectF ArrowButtonsEventBlock::buttonPoses[3] = {QRectF(32 + (40 + 36) * 0, 204, 40, 40),
    ...
    ArrowButtonsEventBlock::ArrowButtonsEventBlock(bool advanced, QGraphicsItem* parent)
        : BlockWithButtons("event", "button", true, parent), advanced(advanced), mode(MODE_ARROW) {
        ...
        // center
        GeometryShapeButton* button =
            new GeometryShapeButton(QRectF(-25, -25, 50, 50), GeometryShapeButton::CIRCULAR_BUTTON, this,
                                    Style::unusedButtonFillColor, Style::unusedButtonStrokeColor);
        ...
        // center
        button = new GeometryShapeButton(QRectF(-25, -25, 50, 50), GeometryShapeButton::CIRCULAR_BUTTON, this,
                                         Style::unusedButtonFillColor, Style::unusedButtonStrokeColor);
        ...
        for(int row = 0; row < 4; ++row)
            for(int col = 0; col < 3; ++col) {
                button = new GeometryShapeButton(QRectF(0, 0, 48, 30), GeometryShapeButton::RECTANGULAR_BUTTON, this,
                                                 Style::unusedButtonFillColor, Style::unusedButtonStrokeColor);
    ...
    int ArrowButtonsEventBlock::getSelectedRCKeypadButton() const {
        for(size_t i = 10; i < size_t(buttons.size()); ++i)
            if(buttons[i]->getValue())
                return i - 10;
        return -1;
    }
    ...
    void ArrowButtonsEventBlock::ensureSingleRCKeypadButtonSelected(int current) {
        for(auto i = 10; i < buttons.size(); ++i)
    ...
    ProxEventBlock::ProxEventBlock(bool advanced, QGraphicsItem* parent)
        : BlockWithButtonsAndRange("event", "prox", true, PIXEL_TO_VAL_SQUARE, 700, 4000, 1000, 2000, Qt::black,
                                   Qt::white, advanced, parent) {
        ...
        for(int i = 0; i < 5; ++i) {
            GeometryShapeButton* button =
                new GeometryShapeButton(QRectF(-16, -16, 32, 32), GeometryShapeButton::RECTANGULAR_BUTTON, this,
                                        Style::unusedButtonFillColor, Style::unusedButtonStrokeColor);

            const qreal offset = (qreal)2 - i;
            button->setRotation(-20 * offset);
            button->setPos(128 - 150 * qSin(0.34906585 * offset), 175 - 150 * qCos(0.34906585 * offset));
    ...
    ProxGroundEventBlock::ProxGroundEventBlock(bool advanced, QGraphicsItem* parent)
        : BlockWithButtonsAndRange("event", "proxground", false, PIXEL_TO_VAL_LINEAR, 0, 1023, 400, 450, Qt::black,
                                   Qt::white, advanced, parent) {
        buttonsCountSimple = 3;
    ...
    // Acc Event
    const QRectF AccEventBlock::buttonPoses[3] = {QRectF(32 + (40 + 36) * 0, 200, 40, 40),
    ...
    // Clap Event
    ClapEventBlock::ClapEventBlock(QGraphicsItem* parent) : BlockWithNoValues("event", "clap", parent) {
        new QGraphicsSvgItem(":/images/thymioclap.svgz", this);
    }


    // TimeoutEventBlock
    TimeoutEventBlock::TimeoutEventBlock(QGraphicsItem* parent) : BlockWithNoValues("event", "timeout", parent) {
