...
namespace ThymioVPL {
    ...
    //! The multiplicative factor to apply to the color saturation (in HSV) of the block drop area
    //! when it is not highlighted
    const qreal Style::blockDropAreaSaturationFactor = 0.7;

    //! The multiplicative factor to apply to the color value (in HSV) of the block drop area when
    ...
    static struct PrivateStyle {
        ...
        PrivateStyle()
            : eventColors(ColorList()
                          //<< QColor(0,191,255)
                          << QColor(246, 135, 31) << QColor(155, 48, 255) << QColor(67, 205, 128) << QColor(255, 215, 0)
                          << QColor(255, 97, 3) << QColor(125, 158, 192))
            , stateColors(ColorList() << QColor(0, 218, 3) << QColor(122, 204, 0) << QColor(122, 204, 0)
                                      << QColor(122, 204, 0) << QColor(122, 204, 0) << QColor(122, 204, 0))
            , actionColors(ColorList()
                           //<< QColor(218,112,214)
                           << QColor(0, 150, 219) << QColor(159, 182, 205) << QColor(0, 197, 205) << QColor(255, 99, 71)
                           << QColor(142, 56, 142) << QColor(56, 142, 142))
            , currentColor(0) {
