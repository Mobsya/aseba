...
struct ZeroconfCallbacks {
    static void DNSSD_API registerReply(DNSServiceRef sdRef, DNSServiceFlags flags, DNSServiceErrorType errorCode,
                                        const char* name, const char* regtype, const char* domain, void* context);
    static void DNSSD_API browseReply(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex,
                                      DNSServiceErrorType errorCode, const char* serviceName, const char* regtype,
                                      const char* replyDomain, void* context);
    static void DNSSD_API resolveReply(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex,
                                       DNSServiceErrorType errorCode, const char* fullname, const char* hosttarget,
                                       uint16_t port,
                                       /* In network byte order */ uint16_t txtLen, const unsigned char* txtRecord,
                                       void* context);
};

const Zeroconf::Target::StateNameMap Zeroconf::Target::stateToString{{State::UNINITIALIZED, "uninitialized"},
    ...
    if(targetIt != targets.end()) {
        ...
        if(target.state != Target::State::REGISTERED && target.state != Target::State::REGISTRATING)
            throw Zeroconf::Error(
                FormatableString("forget: target in invalid state: %0").arg(target.stateToString.at(target.state)));
...
void DNSSD_API ZeroconfCallbacks::registerReply(DNSServiceRef sdRef, DNSServiceFlags flags,
...
void Zeroconf::updateTarget(Zeroconf::Target& target, const TxtRecord& txtrec) {
    ...
    if(target.state != Target::State::REGISTERED)
        throw Zeroconf::Error(
            FormatableString("updateTarget: target in invalid state: %0").arg(target.stateToString.at(target.state)));
    ...
    DNSServiceErrorType err = DNSServiceUpdateRecord(target.serviceRef,
                                                     ...
                                                     0,        // no flags
                                                     rawdata.length(), rawdata.c_str(), 0);
    if(err != kDNSServiceErr_NoError)
...
void Zeroconf::browse() {
    ...
    auto err = DNSServiceBrowse(&browseServiceRef,
                                ...
                                "_aseba._tcp",
                                nullptr,  // use default domain, usually "local."
                                ZeroconfCallbacks::browseReply, this);
    if(err != kDNSServiceErr_NoError)
...
void DNSSD_API ZeroconfCallbacks::browseReply(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex,
                                              DNSServiceErrorType errorCode, const char* name, const char* regtype,
                                              const char* domain, void* context) {
    if(errorCode != kDNSServiceErr_NoError) {
...
void Zeroconf::resolveTarget(const std::string& name, const std::string& regtype, const std::string& domain) {
    ...
    auto err = DNSServiceResolve(&target.serviceRef,
                                 0,  // no flags
                                 0,  // default all interfaces
                                 name.c_str(), regtype.c_str(), domain.c_str(), ZeroconfCallbacks::resolveReply, this);
...
void DNSSD_API ZeroconfCallbacks::resolveReply(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex,
...
//! Return an iterator to the target holding a given service reference, return targets.end() if not
//! found.
