...
public:
    ...
    //! We do not care much of the O(n) performance when fetching a target as this vector is small
    //! and the lookup infrequent.
    ...
    //! Compute whether two TargetInformation objects are equal, using data from their parent
    //! TargetKey
    ...
    */
    class TxtRecord {
    public:
        //! The fields are contained as a map of strings to strings, the constraints are enforced
        //! through the assign functions.
...
protected:
    ...
    //! Watch the file description associated with the service reference and call
    //! DNSServiceProcessResult when data are available. The service reference must be valid.
    virtual void processServiceRef(DNSServiceRef serviceRef) = 0;
    //! Stop watching the file description associated with the service reference, and deallocate it
    //! afterwards (calling DNSServiceRefDeallocate). If the service reference is a nullptr, do
    //! nothing.
...
protected:
    ...
    //! A list of all targets currently being processed, i.e. whose serviceRefs are handled by
    //! subclasses This is mostly targets being advertised, but also targets being resolved.
    Targets targets;
