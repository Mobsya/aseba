...
AboutBox::AboutBox(QWidget* parent, const Parameters& parameters) : QDialog(parent) {
    ...
    const QString aboutText = "<p>" + parameters.description + "</p>" +
        (parameters.helpUrl.isEmpty() ? "" :
                 ...
                 "</p>") +
        "<p>" +
        tr("This program is part of Aseba, a set of tools which allow beginners to program robots "
           "easily and efficiently. For more information about Aseba, visit %1.")
    ...
    for(const auto& personList :
        ...
        if(personList.second == &thankToList) {
            ...
            const Grants grants = {{"Swiss National Center of Competence in Research", "", "Robotics"},
                                   ...
                                   {"Swiss Commission for Technology and Innovation", "17479.2",
                                    "Combining Augmented Reality and Robotics for Innovative Accessible Educational "
                                    "Tools"}};
    ...
    const QString asebaBuildInfo =
        tr("build version %1, protocol version %2").arg(ASEBA_REVISION).arg(ASEBA_PROTOCOL_VERSION);
