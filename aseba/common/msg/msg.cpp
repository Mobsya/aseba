...
} messageTypesInitializer;  //!< static instance, used only to have its constructor called on
                            //!< startup
...
bool operator==(const Message& lhs, const Message& rhs) {
    return lhs.source == rhs.source && lhs.type == rhs.type;
}
...
void Message::SerializationBuffer::add(const string& val) {
    if(val.length() > 255) {
        cerr << "Message::SerializationBuffer::add<string>() : fatal error, string length exceeds "
                "255 characters.\n";
...
void UserMessage::deserializeSpecific(SerializationBuffer& buffer) {
    if(buffer.rawData.size() % 2 != 0) {
        cerr << "UserMessage::deserializeSpecific(SerializationBuffer& buffer) : fatal error: odd "
                "size.\n";
...
GetVariables::GetVariables(uint16_t dest, uint16_t start, uint16_t length)
    : CmdMessage(ASEBA_MESSAGE_GET_VARIABLES, dest), start(start), length(length) {}
...
SetVariables::SetVariables(uint16_t dest, uint16_t start, VariablesDataVector variables)
    : CmdMessage(ASEBA_MESSAGE_SET_VARIABLES, dest), start(start), variables(std::move(variables)) {}
