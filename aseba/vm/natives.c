...
static const int16_t aseba_atan_table[20 * 8 + 1] = {
        ...
        if(x == 0) {
            ...
            }
            {
                ...
                if(index < 0) {
                    ...
                    res = __builtin_divsd(__builtin_mulss(aseba_atan_table[index * 8 + bin], 32 - delta) +
                                              __builtin_mulss(aseba_atan_table[index * 8 + bin + 1], delta),
                                          32);
                }
...

            {
                ...
                if(index < 0) {
                    ...
                    int16_t bin = subprecision_index >> 5;
                    int16_t delta = subprecision_index & 0x1f;
                    res = (int16_t)(((int32_t)aseba_atan_table[index * 8 + bin] * (int32_t)(32 - delta) +
                                     (int32_t)aseba_atan_table[index * 8 + bin + 1] * (int32_t)delta) >>
                                    5);
                }
#endif
...
static const int16_t aseba_sin_table[128 + 1] = {
...
// do the sinus of an "aseba" angle that spans the whole 16 bits range, and return a 1.15 fixed
// point value
int16_t aseba_sin(int16_t angle) {
    ...
    {
        int16_t result = (int16_t)(((int32_t)aseba_sin_table[index] * (int32_t)(128 - subIndex) +
                                    (int32_t)aseba_sin_table[index + 1] * (int32_t)(subIndex)) >>
                                   7);
...
// do the cos of an "aseba" angle that spans the whole 16 bits range, and return a 1.15 fixed point
// value
...
const AsebaNativeFunctionDescription AsebaNativeDescription_veccopy = {"math.copy",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_vecfill = {"math.fill",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_vecsort = {"math.sort",
                                                                       "sort array in place",
                                                                       {{-1, "array"}, {0, 0}}};
...
const AsebaNativeFunctionDescription AsebaNativeDescription_mathsin = {"math.sin",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_mathcos = {"math.cos",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_mathrot2 = {"math.rot2",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_mathsqrt = {"math.sqrt",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_rand = {"math.rand",
...
//			vm->variables[dest + 2 + ((dq_start + k) % dq_capacity)] = vm->variables[dest + 2 +
//((dq_start + k + src_length) % dq_capacity)];
...
//			vm->variables[dest + 2 + ((dq_start + k) % dq_capacity)] = vm->variables[dest + 2 +
//((dq_start + k - 1) % dq_capacity)];
// Erase in right half: shift prefix elements left
//		for (k = index_val; k < dq_size - 1; k++)
//			vm->variables[dest + 2 + ((dq_start + k) % dq_capacity)] = vm->variables[dest + 2 +
//((dq_start + k + 1) % dq_capacity)];

static void deque_shift(AsebaVMState* vm, uint16_t dq, uint16_t dq_capacity, uint16_t target, int16_t last,
                        int16_t delta) {
...
const AsebaNativeFunctionDescription AsebaNativeDescription_deqsize = {"deque.size",
                                                                       "report size of dest",
                                                                       {{-1, "deque"}, {1, "size"}, {0, 0}}};

static void _AsebaNative_deqget(AsebaVMState* vm, uint16_t dest, uint16_t deque, uint16_t index_val,
                                uint16_t dest_length, uint16_t deque_length) {
...
static void _AsebaNative_deqinsert(AsebaVMState* vm, uint16_t deque, uint16_t src, uint16_t index_val,
                                   uint16_t deque_length, uint16_t src_length) {
    // infer deque parameters
    uint16_t dq_size = vm->variables[deque];
    uint16_t dq_start = vm->variables[deque + 1];
    uint16_t dq_capacity = deque_length - 2;
    ...
    if(index_val < dq_size / 2) {
        vm->variables[deque + 1] = dq_start =
            (dq_start == 0) ? dq_capacity - src_length : (dq_start - src_length + dq_capacity) % dq_capacity;
        deque_shift(vm, deque, dq_capacity,
                    /* from */ dq_start,
                    /* to   */ index_val - 1 + src_length - 1, src_length);
    }
    // else in right half, shift suffix elements right
    else {
        deque_shift(vm, deque, dq_capacity,
                    /* from */ dq_start + dq_size + src_length - 1,
                    /* to   */ dq_start + index_val + src_length, -src_length);
    }
...
static void _AsebaNative_deqerase(AsebaVMState* vm, uint16_t deque, uint16_t index_val, uint16_t len_val,
                                  uint16_t deque_length) {
    // infer deque parameters
    uint16_t dq_size = vm->variables[deque];
    uint16_t dq_start = vm->variables[deque + 1];
    uint16_t dq_capacity = deque_length - 2;
    ...
    // else in right half, shift suffix elements left
        ...
        deque_shift(vm, deque, dq_capacity,
                    ...
                    /* to   */ dq_start + dq_size - 1 - len_val, len_val);
    }
...
const AsebaNativeFunctionDescription AsebaNativeDescription_deqpushfront = {"deque.push_front",
...
const AsebaNativeFunctionDescription AsebaNativeDescription_deqpushback = {"deque.push_back",
...
void AsebaNative_deqpopback(AsebaVMState* vm) {
    ...
    uint16_t index_val = vm->variables[deque];  // length of deque is index value, from which
                                                // tuple length will be subtracted
