...
/*! Setup the execution status of the VM.
    ...
    nodeId and bytecode, variables, and stack along with their sizes must be set outside this
   function. The content of the variable array is zeroed by this function.
*/
...
/*! Can be called by glue code (including native functions), to stop vm and emit a node specific
 * error */
...
/*! Called by AsebaVMDebugMessage when VM must write its bytecode to flash, write an empty function
 * to leave feature unsupported */
void AsebaWriteBytecode(AsebaVMState* vm);

/*! Called by AsebaVMDebugMessage when VM must restart the node and enter to the bootloader, write
 * an empty function to leave feature unsupported */
void AsebaResetIntoBootloader(AsebaVMState* vm);

/*! Called by AsebaVMDebugMessage when VM must put to node in deep sleep. Write an empty function to
 * leave feature unsupported */
