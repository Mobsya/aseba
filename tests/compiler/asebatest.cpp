...
static const struct option long_options[] = {
    {"fail", no_argument, nullptr, 'f'},        {"comp_fail", no_argument, nullptr, 'c'},
    {"exec_fail", no_argument, nullptr, 'e'},   {"post_fail", no_argument, nullptr, 'p'},
...
int main(int argc, char** argv) {
    ...
    const bool should_any_fail(should_compilation_fail || should_execution_fail || should_postexecution_fail ||
                               should_memcmp_fail);
    ...
    checkForError("PostInitExecution", should_postexecution_fail, stillExecuting,
...
std::wstring read_source(const std::string& filename) {
    ...
    /*for (size_t i = 0; i < utf8Source.length(); ++i)
        std::cerr << "source char " << i << " is 0x" << std::hex << (unsigned)(unsigned
       char)utf8Source[i] << " (" << char(utf8Source[i]) << ")" << std::endl;
    */
    const std::wstring s = UTF8ToWString(utf8Source);

    /*std::cerr << "len utf8 " << utf8Source.length() << " len final " << s.length() << std::endl;
    for (size_t i = 0; i < s.length(); ++i)
        std::wcerr << "dest char " << i << " is 0x" << std::hex << (unsigned)s[i] << " (" << s[i] <<
    ")"<< std::endl;
    */
    return s;
}
