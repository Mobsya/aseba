...
//! Make sure only one program runs at a time, because of global world pointer for Aseba C native
//! functions
...
struct QtSimulatorEnvironment : SimulatorEnvironment {
    ...
    virtual void notify(const EnvironmentNotificationType type, const std::string& description,
        ...
        transform(arguments.begin(), arguments.end(), back_inserter(qArguments),
                  [](const string& stdString) { return QString::fromStdString(stdString); });
        emit parent->notify(level, QString::fromStdString(description), qArguments);
    }

    virtual std::string getSDFilePath(const std::string& robotName,
                                      unsigned fileNumber) const override {
...
//! An helper object to de-register the simulator environment once its content gets destroyed at end
//! of scope
...
template<typename T>
std::tuple<QString, T> getValue(const QVariantMap& v, const QString& name,
                                const QString& context = "Scenario") {
    if(!v.contains(name))
        return std::make_tuple(QString("%1 is missing property \"%2\"").arg(context).arg(name),
                               T());
    const auto& entry(v.value(name));
    if(!entry.canConvert<T>())
        return std::make_tuple(QString("%1 property \"%2\" is not of type %3")
                                   .arg(context)
                                   .arg(name)
                                   .arg(typeid(T).name()),
                               T());
...
ThymioRobotInterface::ThymioRobotInterface(Enki::DirectAsebaThymio2& thymio) : thymio(thymio) {}
...
QVariantList ThymioRobotInterface::nativeCalls() const {
    ...
    for(const auto& nativeCallEntry : thymio.thymioNativeCallLog) {
        ...
        transform(nativeCallEntry.second.begin(), nativeCallEntry.second.end(),
                  back_inserter(qValues), [](int16_t value) { return QVariant(value); });
...
QString Simulator::runProgram(const QVariantMap& scenario, const QVariantMap& events,
                              const QString& source, QJSValue callback) const {
    ...
    if(scenario.contains("groundTexture")) {
        ...
        if(!image.isNull()) {
            ...
            std::copy(imageData, imageData + image.width() * image.height(),
                      std::back_inserter(groundTexture.data));
    ...
    if(nodeId != 1) {
        qCritical()
            << "nodes manager did not return the right nodeId for \"thymio-II\", should be 1, was "
            << nodeId;
        return "";    // error in code, not in passed objects
    }
    ...
    const bool compilationResult(
        compiler.compile(input, bytecode, allocatedVariablesCount, compilationError));

    if(!compilationResult) {
        return QString("Compilation error: %1")
            .arg(QString::fromStdWString(compilationError.message));
    }
    ...
    for_each(setBytecodeMessages.begin(), setBytecodeMessages.end(),
             [=](unique_ptr<Message>& message) { thymio->inQueue.emplace(message.release()); });
